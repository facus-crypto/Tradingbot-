                ...
                if abs(precio_actual - emas['ema20']) / emas['ema20'] < 0.01:
                    senal = {
                        ...
                        'motivo': f"EMAs alcistas ({emas['tendencia'],
                                                    'stop_loss': sl,
        "motivo": f"EMAs alcistas({emas['tendencia']}) + MACD histograma cruce alcista(hist: {macd_info['histograma']: .6f})"
                                                    'fase_trailing': fase
            ...
            if senal:
                print(
                    f"[AVAXUSDT] 🔍 Señal detectada: {senal['accion']} - {senal['motivo']}")
                return senal
