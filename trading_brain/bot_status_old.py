...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        status_msg += "• Tiempo activo: {}h {}m\n".format(horas, minutos)
        status_msg += "• Ciclos completados: {}\n".format(stats['ciclos_completados'])
        status_msg += "• Señales enviadas: {}\n".format(stats['señales_enviadas'])
        status_msg += "• Operaciones activas: {}\n".format(stats['operaciones_activas'])
        status_msg += "• Último ciclo: {}\n".format(stats['ultimo_ciclo'][11:19])
        ...
        precios_col2 = []  # Columna derecha (últimos 5: ADA, AVAX, XRP, DOT, ATOM)
        ...
        for i, (nombre, par) in enumerate(pares):
            try:
                ...
                if precio is None:
                    ...
                    # Formatear precio según su valor (igual que antes)
                    if precio > 1000:
                        ...
                        # Para los pequeños: 4 decimales
                        precio_str = f"${precio:.4f}"
                
                linea = f"• {nombre}: {precio_str}"
                ...
                linea = f"• {nombre}: Error"
                if i < 5:
                    precios_col1.append(linea)
                else:
                    precios_col2.append(linea)

        # Unir las dos columnas con alineación
        for j in range(5):
            # Añadir espacios a la columna izquierda para alineación
