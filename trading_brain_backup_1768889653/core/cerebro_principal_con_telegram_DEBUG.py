...
Conecta: Cerebros → Telegram → Confirmación → Risk Manager
"""
import asyncio
import logging
...
try:
    ...
    print(f"⚠️  No InterfazTelegram: {e}")
...
class SistemaCompleto:
    ...
    def __init__(self, usar_telegram=True):
        ...
        if usar_telegram and TELEGRAM_DISPONIBLE:
            ...
            logger.info("📱 Interfaz Telegram integrada al sistema")
    ...
    async def ciclo_completo(self, ciclos=3, intervalo_segundos=30):
        ...
        logger.info(f"🔄 INICIANDO CICLO COMPLETO ({ciclos} ciclos)")
        
        for ciclo in range(1, ciclos + 1):
            try:
                logger.info(f"\n{'='*60}")
                logger.info(f"🌀 CICLO {ciclo}/{ciclos}")
                logger.info(f"{'='*60}")
                
                # 1. Análisis con todos los cerebros
        ...
        if tasks:
            ...
            for i, resultado in enumerate(resultados):
                ...
                if isinstance(resultado, Exception):
                    ...
                    logger.info(f"  📡 {simbolo}: Señal detectada (Fuerza: {resultado.get('fuerza', 0)}/10)")
                else:
    ...
    async def procesar_senales(self, senales: List[Dict]):
        """Procesa señales encontradas"""
        logger.info(f"\n📊 PROCESANDO {len(senales)} SEÑAL(ES)")
        
        for senal in senales:
            # Filtrar por fuerza mínima
            if senal.get('fuerza', 0) < 6:
                logger.info(f"  ⚠️  {senal.get('simbolo')}: Fuerza insuficiente ({senal.get('fuerza')}/10)")
                continue
            ...
            if self.interfaz_telegram:
                try:
                    ...
                    if enviado:
                        ...
                        logger.info(f"  📤 {senal.get('simbolo')}: Enviada a Telegram")
                    else:
                        logger.warning(f"  ❌ {senal.get('simbolo')}: Error enviando a Telegram")
                except Exception as e:
                    logger.error(f"  💥 {senal.get('simbolo')}: Error Telegram: {e}")
            else:
                # Mostrar señal directamente
                logger.info(f"  🔔 {senal.get('simbolo')}: {senal.get('direccion')} "
    ...
    def mostrar_estado_sistema(self, ciclo: int):
        ...
        logger.info(f"\n📈 ESTADO DEL SISTEMA (Ciclo {ciclo}):")
        logger.info(f"  • Cerebros activos: {len(self.cerebros_especializados)}")
        logger.info(f"  • Señales enviadas: {self.senales_enviadas}")
        logger.info(f"  • Telegram: {'✅ CONECTADO' if self.interfaz_telegram else '❌ NO DISPONIBLE'}")
        
        if self.interfaz_telegram:
            estado_tg = self.interfaz_telegram.get_estado()
            logger.info(f"  • Señales pendientes Telegram: {estado_tg.get('senales_pendientes', 0)}")
    
    def mostrar_resumen_final(self):
        """Muestra resumen final"""
        logger.info(f"\n{'⭐'*20} RESUMEN FINAL {'⭐'*20}")
        logger.info(f"  • Total ciclos completados: ✓")
        logger.info(f"  • Cerebros operativos: {len(self.cerebros_especializados)}")
        logger.info(f"  • Señales detectadas totales: {self.senales_enviadas}")
        logger.info(f"  • Sistema integrado: ✅ COMPLETO")
        logger.info(f"\n🎯 SISTEMA LISTO PARA OPERAR:")
        logger.info(f"  1. Cerebros especializados → Analizan mercado")
        logger.info(f"  2. Filtran señales (fuerza ≥ 6/10)")
        logger.info(f"  3. Envían a Telegram (simulación)")
        logger.info(f"  4. Esperan confirmación manual")
        logger.info(f"  5. Risk Manager calcula posición (25% wallet, x2)")
        logger.info(f"\n⚠️  PARA PRODUCCIÓN:")
        logger.info(f"  • Configurar Token y Chat ID real en telegram_signal.py")
        logger.info(f"  • Configurar API Keys de Binance en config.py")
        logger.info(f"  • Cambiar modo_prueba=False")
        logger.info(f"{'✅'*20} FINALIZADO {'✅'*20}")

# Función principal
async def main():
    """Función principal"""
    print("\n" + "="*70)
    print("🚀 SISTEMA DE TRADING INSTITUCIONAL - PRUEBA COMPLETA")
    print("="*70)
